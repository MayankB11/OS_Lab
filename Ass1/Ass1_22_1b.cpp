...
int main()
{
	...
	while(1)
	{
		...
		if(pid==0) 
		{
			if(execvp(argc[0],argc)<0){ 									// Exec command in the child process
			cout<<"Command not found"<<endl;
			break;		
			}
		}
	    
	        waitpid (pid, &status, 0);				// Wait till the child process is finished(i.e. wait before getting another command)
